...
namespace CompactOT.DataStructures
{
    public abstract class PackedArray<T> : IReadOnlyList<T>, ICollection<T>, ICollection
    {
        ...
        protected PackedArray(int numberOfBytes, int numberOfElements) // todo(lumip): change to numberOfElements and bitSizeOfElements
        {
        ...
        protected PackedArray(byte[] bytes, int numberOfBytes, int numberOfElements)
        {
            ...
            _buffer = bytes; // note(lumip): Change to CompactMPC: not copying 'bytes' but using it directly.
            _length = numberOfElements;
        }
